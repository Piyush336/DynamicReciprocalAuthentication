...
public class upload1 extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql = "insert into files (fileid,filename,file_key,ownerid)values(null,?,?,?)";
			...
			ResultSet rs=con.createStatement().executeQuery("select count(*) from files where filename='"+itemName+"'");
			...
			if(isHaving)
			{
				...
				if (FileUpload.fileUpload(path)) {
					...
					response.sendRedirect("adminpage.jsp?msgg= sucess..!");
				} else {
